...
class DQNet(object):
    def __init__(self, state_size, action_size, learning_rate, name='DQNetwork'):
        ...
        with tf.variable_scope(name):
            ...
            self.conv2_out=tf.nn.relu(self.conv2, name="conv2_out")
            self.conv3 = tf.layers.conv2d(inputs=self.conv2_out, filters=64, kernel_size=[3,3],strides=[2,2], padding="VALID", kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(), name='conv3')
            self.conv3_out = tf.nn.relu(self.conv3, name="conv3_out")
            
            self.flatten = tf.contrib.layers.flatten(self.conv3_out)
            
            self.fc = tf.layers.dense(inputs=self.flatten, units=512, activation=tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer())
...
class DDQNet(object):
    def __init__(self, state_size, action_size, learning_rate, name='DQNetwork'):
        ...
        with tf.variable_scope(name):
            ...
            self.fc_statevalue = tf.layers.dense(inputs=self.flatten, units=512, activation=tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer())
            self.fc_advantage_update = tf.layers.dense(inputs=self.flatten, units=512, activation=tf.nn.elu,kernel_initializer=tf.contrib.layers.xavier_initializer())
            self.output_statevalue = tf.layers.dense(inputs=self.fc_statevalue, units=1, activation=tf.nn.elu,kernel_initializer=tf.contrib.layers.xavier_initializer()) 
            self.output_advantage_update = tf.layers.dense(inputs=self.fc_advantage_update, units=self.action_size, activation=tf.nn.elu,kernel_initializer=tf.contrib.layers.xavier_initializer())
   
            #the layer where the state-value and the advantage update are combined to one vector
            self.output = self.output_statevalue + tf.subtract(self.output_advantage_update, tf.reduce_mean(self.output_advantage_update, axis=1, keepdims=True)) 
...
elif network_flag == 'DDQN':
    DQNetwork = DDQNet(state_size, action_size, learning_rate, name="DQNetwork")
    ...
    with tf.Session() as sess:
        ...
        for episode in range(total_episodes):
            ...
            while step < max_steps:
                ...
                if death == {'ale.lives': 0}:
                    print("Punished for the last death!")
                    reward -= 1000
...
def train_per(stacked_frames):
    ...
    with tf.Session() as sess:
        ...
        #save_performance_statistics(episodes_stats, steps_per_episode_stats, total_rewards_per_episode_stats)
        print('Perfromance Statistics saved!')
